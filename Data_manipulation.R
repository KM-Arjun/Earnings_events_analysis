...
# Price data from Bloomberg and Earnings announcement date-time data from Wall Street Horizon 
...
# All events are broadly classified into BMO(Before Market Open) and AMC(After Market Close) 
# The return and the implied volatility calculation will be affected by this classification 
# So we classify the events into BMO or AMC based on when the earnings is announced 

# Function to calculate log returns for BMO events
...
# Create a new column to store the log returns over the earnings event 
df_2009 <- df_2009 %>% 
  rowwise() %>% 
...
#Now we create a column for the 6-month implied vol at the money and the 1-month implied vol at the money before the announcement date.

#Function to get the implied volatility for BMO events
calc_base_vol_BMO <- function(stock, ann_date){
  ...
  implied_vol <- prev_close 
  return(implied_vol)
}


#Function to get the implied volatility for AMC events
...
# Create a column for 1 month implied vol before the earnings release in 2009
df_2009 <- df_2009 %>% 
  rowwise() %>% 
...
# Create a column for 1 month implied vol before the earnings release in 2014
df_2014 <- df_2014 %>% 
  rowwise() %>% 
  mutate(impvol_1month_ATM = ifelse(time_class == 'BMO',
                             calc_base_vol_BMO(stock_symbol, announce_date),
...
# Create a column for 1 month implied vol before the earnings release in 2019
df_2019 <- df_2019 %>% 
  rowwise() %>% 
  mutate(impvol_1month_ATM = ifelse(time_class == 'BMO',
                             calc_base_vol_BMO(stock_symbol, announce_date),
...
#Save the updated tables to a csv
